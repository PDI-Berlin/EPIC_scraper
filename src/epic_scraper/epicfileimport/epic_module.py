...
def epiclog_read(name):
    ...
    df.columns=df.columns.str.replace('[\']','', regex=True).str.replace('[.]','_', regex=True).str.replace('[ ]','_', regex=True)

    # Converting columns named 'Date&Time' to 'Date', as it is inconsistent between different log files.
    ...
    # Add comment and name attributes to the DataFrame from log files and replace dots and spaces with underscores.
    df.comment = open(name,'r').readlines()[0][1:].replace(".", "_")
...
def threshold_sampling(df, change, threshold_value = 0.01):
    '''
    Using pct_change/diff to find relative/absolute difference between two rows in a column.
    This function is designed to reduce the unnecessary data, rather than resampling.
    Yet, if you change the value threshold_value on your needs, you can use it as resampling
    anyway.
    The threshold_value 0.01 is an arbitrary value, but otherwise for loop that is used inside
    accumulated_sampling function is too heavy computational-wise.
    '''
    ...
    if change == 'relative':
        df = pd.merge(df.reset_index(), df.pct_change().reset_index(), on=['Date']).set_index('Date').loc[(df.pct_change()!=0).any(axis=1)].fillna(threshold_value+1)
    else:
        df = pd.merge(df.reset_index(), df.diff().reset_index(), on=['Date']).set_index('Date').loc[(df.diff()!=0).any(axis=1)].fillna(threshold_value+1)
...
def accumulated_sampling(df, change, threshold_value):
    ...
    just an empty DataFrame. Rather this method creates a new DataFrame(dataframe_change),
    ...
    if df.empty != True:
        ...
        for j in range(len(df)):
            if change == 'relative':
                condition = abs(pd.Series([dataframe_change.iloc[-1,0],df.iloc[j,0]]).pct_change()[1]).round(1) * 100
            else:
                condition = abs(pd.Series([dataframe_change.iloc[-1,0],df.iloc[j,0]]).diff()[1]).round(1)
...
def resampling(dataframe_list, percent_cut, value_cut, resampling_period, resample_method = 'diff'):
    '''
    ...
    for i in range(len(dataframe_list)):
        ...
        if resample_method == 'diff':
            ...
            if dataframe_list[i].columns.size < 3:
                ...
                if dataframe_list[i].filter(regex='IG|MIG|PG').columns.values.tolist():
                    dataframe_list[i] = threshold_sampling(dataframe_list[i], 'relative')
                    dataframe_list[i] = accumulated_sampling(dataframe_list[i], 'relative', percent_cut)
                ...
                if dataframe_list[i].filter(regex='PID|Pyro').columns.values.tolist():
                    dataframe_list[i] = threshold_sampling(dataframe_list[i], 'absolute')
                    dataframe_list[i] = accumulated_sampling(dataframe_list[i], 'absolute', value_cut)
        else:
        ...
        if dataframe_list[i].filter(regex='Message').columns.values.tolist():
            ...
            dataframe_list[-1].grow = "Error: No Message log detected, can not determine the number of growth events and the start and end of the growth!"
...
def growth_time(df):
    ...
    if df['Message'].str.contains('moved from').any() == True:
        ...
        # When the Mirror values dropped, the DataFrame can be empty(No growth detected). Check this to prevent errors.
        if df.empty != True:
            # Split the Message column to Object, From and to Columns
            df[["object", "from"]] = df.pop("Message").str.split(" moved from ", expand=True)
            ...
            if any(count != 2 for count in df['object'].value_counts()):
                if any(count > 1 for count in df['object'].value_counts()):
                    ...
                    # in here there needs to be a check again for the case if for the given growth event, in the from column GC comes before
                    # MC_manip, a growth started in previous day, vice versa means a growth started today and still going on.
                    df.grow = "The number of growth events is not equal to the number of start and end of the growth events!"
            else:
                # in here there needs to be a check again for the case if for the given growth event, in the from column GC comes before
                # MC_manip, which should raise an error as it means that there is a user error: a growth started in previous day
                # and the it is done today and user tries to start a new growth event today with the same name.
                if len(df.index) == 2:
                    df.grow = "Single Growth detected."
                    print("Start of the Growth: " + df.index[0].strftime("%Y-%m-%d %H:%M:%S"))
                    print("End of the Growth: " + df.index[-1].strftime("%Y-%m-%d %H:%M:%S"))
                if len(df.index) % 2 == 0:
                    df.grow = str(df['object'].value_counts()[0]) + " growths detected with the names " + ','.join(df['object'].value_counts().index.values.tolist()) + "."
                    for i in range(0, len(df.index), 2):
                        print("Start of the Growth " + df.iloc[i,0] + ":" + df.index[i].strftime("%Y-%m-%d %H:%M:%S"))
                        print("End of the Growth " + df.iloc[i+1,0] + ":" + df.index[i+1].strftime("%Y-%m-%d %H:%M:%S"))
        else:

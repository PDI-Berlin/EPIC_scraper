...
def epiclog_read_batch(date, data_path):
    ...
    dataframe_list = [
        epiclog_read(path)
        if not any(excluded in path for excluded in excluded_files)
        else None
...
def extract_growth_messages(folder_path, message_filename):
    ...
    Parameters:
    folder_path (str): The directory path where the Messages.txt file is located.

    Returns:
    growth_id (str): An identifier for the growth process.
...
def epic_hdf5_exporter(hdf_file, dataframe_list, start_datetime):
    """
    Exports a list of dataframes to an HDF5 file.
    Parameters:
    file (str): The file name including the directory path where
    the HDF5 file will be saved.
    dataframe_list (list): A list of pandas DataFrames to be exported in hdf5 datasets.
    start_datetime (datetime): The start datetime used to calculate the time axis
    for the datasets.

    Note: growth_id and start_datetime can be extracted from the Messages.txt file
